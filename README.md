...
¹¹ **Mac M5 — re-measured 2026-09-09 (the same day as the letterbox fix) against the corrected preprocessing** (superseding the original 2026-08-08 session's numbers, which read 7.41/7.53/8.24 ms and ~0.3595/0.3594 mAP). mAP dropped ~0.0018–0.0019 across all three — see [Benchmark Methodology](#benchmark-methodology) for why removing the upscale bug moved mAP *away* from the published baseline. Latency also shifted (+1.1 to +2.0 ms per combination) — unrelated to the preprocessing fix (latency profiling uses a fixed synthetic input, not COCO images) and attributed to ordinary session-to-session machine-state variance, the same category as the Fedora CPU thermal variance documented in [Emergent Findings](#emergent-findings), not yet characterised across repeat Mac sessions the way Fedora's was across three.

**Note (2026-09-12):** unlike Fedora's three sessions (confirmed identical Python/torch/onnxruntime/numpy across all three, directly verified from each result file), this "ordinary machine-state variance" attribution for Mac isn't independently verified the same way — the original 2026-08-08 session's result file was overwritten in place rather than archived, so its environment can no longer be checked against the 2026-09-09 re-measurement. Likely still valid (same physical local machine both times, not an ephemeral cloud VM), but should be read as an assumption, not a confirmed-clean comparison the way Fedora's is.

**Further note (2026-09-12):** the environment-consistency question above has since been answered through an independent source. The packages relevant to this comparison — torch, onnxruntime, numpy — have install timestamps unchanged since the env's creation on 2026-06-19 (Jun 19 14:53:xx, ~5 minutes after the env directory itself was created, consistent with one continuous setup rather than a later change); both Mac sessions ran on this same, untouched-for-these-packages environment. The env itself was touched once since then, on 2026-09-11, to add unrelated notebook-execution tooling (nbconvert/nbclient/ipykernel) for an unconnected task — confirmed not to touch any package this comparison depends on. Combined with `docs/benchmark-run-1-mac-findings.md` already preserving every measured value from the original session, this is now confirmed with high confidence, not merely a likely assumption. See `docs/issue-log/2026-09-12-mac-env-consistency-resolved-via-filesystem-evidence.md` for the full investigation.

**Corroboration (2026-09-10):** the following day, Colab T4 Run 3 (footnote 10) shows the same-direction, similar-magnitude FP32 mAP shift (−0.0019, landing at the same 0.3576) on entirely different hardware and runtime code — ~24-30 hours apart, against the identical fixed commit, independent evidence this is a genuine preprocessing effect, not a Mac- or MPS-specific artifact.
...
**SUPERSEDED (2026-09-12) — the "inversion" above was itself built on unmatched environments, see the full breakdown further down.** Run 1 and Run 2 — the exact pair this inversion came from — do not share an identical software stack (onnxruntime 1.18.0 vs. 1.26.0, numpy 1.26.4 vs. 2.0.2, crossing the 1.x→2.x major boundary). The "inversion" language above should not be read as two controlled trials of the same precision producing opposite results under identical conditions — it's two runs under two different software environments producing different numbers, which is a different, weaker claim than the original wording suggests.

The underlying observation is more nuanced than "unpredictable," on a closer look at all three runs together: INT8's p95 was faster than FP16's in 2 of 3 runs (Run 1, Run 3), with Run 2 — the same session already flagged above for anomalous CoV — the outlier; but INT8's CoV was higher than FP16's in 2 of 3 runs (Run 2, Run 3), with Run 1 the outlier there instead. That's a weak lean toward "INT8 is usually faster at p95 but with somewhat higher relative variance around it, and Run 2 was anomalous on both counts at once" — more specific than pure unpredictability, but still only three points, not a settled pattern.

**SUPERSEDED (2026-09-12) — do not use for marketing, see the replacement claim below.** The "weak lean" language above treats the three runs as comparable trials differing only by noise. They aren't — none of the three TRT runs shares a matching software stack with either of the other two (full breakdown below). This study cannot support a reliable claim about which precision has more predictable tail latency; see the corrected claim below for what can actually be said.
...
**Correction (2026-09-12):** everything above — the original two-run inversion, the mechanism it was retracted from, and this retraction's own replacement reasoning — still implicitly treated Run 1/Run 2/Run 3 as three comparable trials, differing only in whichever GPU-instance noise produced the results. They aren't fully comparable. Checked each run's own recorded `hardware_info`: no two of the three TRT runs share an identical software stack. Run 1 and Run 2 — the exact pair the original "inversion" came from — already differ in onnxruntime (1.18.0 vs. 1.26.0) and numpy (1.26.4 vs. 2.0.2, crossing the 1.x→2.x major boundary); Run 3 differs further still, in Python, torch, onnxruntime, and numpy all four. So this was never "two clean same-environment runs disagreed, then a third clean run settled it" — that confound was present from the very first comparison, not something Run 3 introduced. The tail-latency observations throughout this finding should be read as three data points collected under three different software environments, not three repeated trials under controlled conditions. (Separately, torch's own resolved CUDA build tag was `cu128` in all three runs — see `docs/issue-log/2026-09-10-colab-onnx-wheel-build-failure.md` for why that doesn't confirm or rule out a host-level CUDA change either way.)

**Corrected claim, safe to use (2026-09-12):** this study cannot make a reliable claim about which precision — INT8 or FP16 — has more predictable tail latency, given that no two of the three TensorRT sessions measured were run under matching software environments. What this study *can* say reliably: both precisions show meaningful session-to-session latency variance on shared Colab infrastructure; the p95 ordering between INT8 and FP16 has flipped in different directions across the three sessions measured (INT8 faster in Run 1, slower in Run 2, faster again in Run 3); and the deployment lesson is that tail-latency behavior must be validated directly on the target deployment hardware and software stack, not assumed from this or any other single benchmark run.
...
**SUPERSEDED (2026-09-12) — same environment-confound issue as the INT8/FP16 tail-latency finding above, not previously flagged here.** Attributing this CV pattern to "CUDA-core clock sensitivity" is a mechanism claim inferred from three sessions that do not share a matching software stack (see the INT8/FP16 correction above for the full breakdown). What's confirmed: TRT FP32 shows meaningful session-to-session latency variance (CV 16.6%–25.6% across three sessions) on shared Colab infrastructure. The specific causal attribution to GPU clock behavior is not confirmed — same confound, same fix.
...
The overlay intentionally does not claim things this study didn't establish: it labels precision "FP32", not "FP16" (CoreML EP FP16/INT8 quantisation was never built — see Runtime and Hardware Scope), and carries no "Neural Engine Accelerated" annotation. That omission decision still holds, though the justification is now more specific than the original latency-only test: direct `powermetrics` measurement (see Runtime and Hardware Scope) found likely real, if partial, Neural Engine engagement, but no deployment-relevant latency benefit from it — not the clean "no engagement" the demo originally assumed. Either way, asserting "accelerated" on screen isn't something this study's data supports. A demo overlay is exactly the kind of place an unverified claim quietly survives.
...
**Note (2026-09-12):** the "CUDA 12.8" figure quoted here and elsewhere in this README (Run 2's pinned versions above, and the file footer) is `torch.version.cuda` — the toolkit tag PyTorch's own wheel was built against, not an independently-confirmed host driver version. See `docs/issue-log/2026-09-10-colab-onnx-wheel-build-failure.md`'s 2026-09-12 correction for the full explanation.
