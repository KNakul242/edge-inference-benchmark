...
> **Hardware coverage:** CPU benchmarks on Fedora Linux 42 (Intel Core Ultra 5 125H) — **Fedora hardware is no longer available to this project; these results are permanently frozen at their 2026-05 values** (see mAP note below). TensorRT benchmarks ran on Colab-hosted Tesla T4 (Run 3, 2026-09-10, against the corrected preprocessing pipeline — `tensorrt` pinned to 10.16.1.11; CUDA/torch/onnxruntime moved with Colab's default image, see Pinned Versions below). Mac M5 (Apple M5 MacBook Air) benchmarks ran 2026-09-09 — the same day as the preprocessing fix — for PyTorch MPS (FP32, FP16) and ONNX Runtime + CoreML EP (FP32) — see [Runtime and Hardware Scope](#runtime-and-hardware-scope) for what remains unbuilt (CoreML EP FP16/INT8).
