...
> **Hardware coverage:** CPU benchmarks on Fedora Linux 42 (Intel Core Ultra 5 125H) — **Fedora hardware is no longer available to this project; these results are permanently frozen at their 2026-05 values** (see mAP note below). TensorRT benchmarks on Colab-hosted Tesla T4 (TRT 10.16.1.11, CUDA 12.8) — pending a cheap re-measurement, not yet done. Mac M5 (Apple M5 MacBook Air) benchmarks re-measured 2026-09-10 for PyTorch MPS (FP32, FP16) and ONNX Runtime + CoreML EP (FP32) against a corrected preprocessing pipeline — see [Runtime and Hardware Scope](#runtime-and-hardware-scope) for what remains unbuilt (CoreML EP FP16/INT8).

> **mAP note:** All mAP values are from this pipeline's evaluation (class-agnostic NMS, eval_conf_threshold=0.001). Published YOLOv8n baseline: 0.372 mAP@0.5:0.95 (Ultralytics, class-aware NMS). **Correction (2026-09-09):** a preprocessing bug (missing letterbox upscale clamp) was found and fixed, affecting ~1-in-5 COCO val2017 images; Mac M5 numbers below reflect the fix, Fedora CPU and Colab T4 numbers do not yet (see per-row footnotes). The gap's composition (NMS vs. preprocessing) is no longer a single constant figure until all three targets are re-measured on the same pipeline. See [Benchmark Methodology](#benchmark-methodology).
